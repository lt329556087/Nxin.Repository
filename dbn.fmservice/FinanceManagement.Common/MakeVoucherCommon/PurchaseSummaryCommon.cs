    ...
    public class PurchaseSummaryCommon : MakeVoucherBase
    {
        ...
        public override FD_SettleReceipt getVoucherList(FM_CarryForwardVoucherODataEntity model, FM_CarryForwardVoucherInterfaceSearchCommand request)
        {
            ...
            List<RptSearchModel> rpts = new List<RptSearchModel>() {
             ...
             new RptSearchModel(){Text="采购摘要",Value=model.TransferAccountsAbstractName},
            ...
            {
                List<PurchaseSummaryResultData> purchaseSummarys = GetPurchaseSummaryDataList(model, request, item, domain, isBreak, rpts);//获取采购汇总表数据
                bool isDebit = false;
                    ...
                    item.Extends?.ForEach(extend =>
                    {
                        switch (extend.Sort)
                        {
                            case (int)SortTypeEnum.商品分类:
                                if (products.FindIndex(s => s.id == extend.Object) < 0)//去重的作用
                                {
            ...
            domain.ProductLst = productsStr + "~" + suppliersStr;
            domain.RptSearchText = JsonConvert.SerializeObject(rpts);
            domain.Lines = Lines;
            return domain;
        }

        private List<PurchaseSummaryResultData> GetPurchaseSummaryDataList(FM_CarryForwardVoucherODataEntity model, FM_CarryForwardVoucherInterfaceSearchCommand request, FM_CarryForwardVoucherDetailODataEntity item, FD_SettleReceipt domain, bool isBreak, List<RptSearchModel> rpts)
        {
            try
            {
                #region 构造接口参数
                PurchaseSummaryRequest purchaserequest = new PurchaseSummaryRequest()
                {
                    ...
                    PurchaseAbstract = model.TransferAccountsAbstract,
                    QiDian = 0,
                    SummaryT1Rank = "-1",
                    SummaryT2Rank = "-1",
                    SummaryT3Rank = "-1",
        ...
        public Dictionary<string, string> GetDict()
        {
            ...
            value.Add("1911081429200000301", "_purchaseAmount");
            value.Add("1911081429200000302", "_adjustAmount");
            value.Add("1911081429200000303", "_purchaseTotalWithinTax");
            value.Add("1911081429200000304", "_amountWithoutTax");
            value.Add("1911081429200000305", "_fare");
            return value;
        }
    }
    public class PurchaseSummaryResultData: EntitySubClass
    {
