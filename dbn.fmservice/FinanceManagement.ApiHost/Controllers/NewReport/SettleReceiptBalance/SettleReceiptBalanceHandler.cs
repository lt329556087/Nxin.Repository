...
namespace FinanceManagement.ApiHost.Controllers.SettleReceiptBalance
{
    ...
    /// 往来余额表
    /// </summary>
    public class SettleReceiptBalanceHandler : IRequestHandler<SettleReceiptBalanceQueryCommand, GatewayResultModel>
    {
        IIdentityService _identityService;
        private readonly HttpClientUtil _httpClientUtil;
        private readonly HostConfiguration _hostConfiguration;

        public SettleReceiptBalanceHandler(IIdentityService identityService, HttpClientUtil httpClientUtil, HostConfiguration hostConfiguration)
        {
        ...
        public async Task<GatewayResultModel> Handle(SettleReceiptBalanceQueryCommand request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                //}
                var url = $"{_hostConfiguration.ReportService}/api/SettleReceiptBalance/GetBalanceReport";
                return await _httpClientUtil.PostJsonAsync<object, GatewayResultModel>(url, request);
            }
            catch (Exception ex)
            {
                result.msg = "请求异常";
                Serilog.Log.Error("SettleReceiptBalanceHandler查询：异常："+ex.ToString()+";param="+JsonConvert.SerializeObject(request));
                return result;
            }
        }
    }
