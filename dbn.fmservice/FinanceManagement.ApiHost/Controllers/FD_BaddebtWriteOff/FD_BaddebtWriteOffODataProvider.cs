...
{
    public class FD_BaddebtWriteOffODataProvider : OneWithManyQueryProvider<FD_BaddebtWriteOffODataEntity, FD_BaddebtWriteOffDetailODataEntity>
    {
        private IIdentityService _identityservice;
        private QlwCrossDbContext _context;

        public FD_BaddebtWriteOffODataProvider(IIdentityService identityservice, QlwCrossDbContext context)
        {
            _identityservice = identityservice;
            _context = context;
        }

        public IEnumerable<FD_BaddebtWriteOffODataEntity> GetList(ODataQueryOptions<FD_BaddebtWriteOffODataEntity> odataqueryoptions, Uri uri)
        {
            var datas = GetData(odataqueryoptions, uri).AsEnumerable();
            return datas;
        }

        public IQueryable<FD_BaddebtWriteOffODataEntity> GetData(ODataQueryOptions<FD_BaddebtWriteOffODataEntity> odataqueryoptions, Uri uri)
        {
            return GetDatas();
        }

        public override IQueryable<FD_BaddebtWriteOffODataEntity> GetDatas(NoneQuery query = null)
        {
            var sql = GetHeadSql();
            sql+=string.Format(@"
                                WHERE A.EnterpriseID= {0} 
                                ORDER BY A.DataDate DESC,A.Number DESC  ", _identityservice.EnterpriseId);
            return _context.FD_BaddebtWriteOffDataSet.FromSqlRaw(sql); 
        }
        private string GetHeadSql()
        {
            return string.Format(@"SELECT  
                                        ...
                                        IFNULL(B.WriteOffAmount,0.0)  WriteOffAmount,
                                        ...
                                        FROM nxin_qlw_business.FD_BaddebtWriteOff A 
                                        LEFT JOIN nxin_qlw_business.FD_BaddebtWriteOffdetail B ON A.NumericalOrder=B.NumericalOrder
        ...
        /// <returns></returns>
        public override Task<FD_BaddebtWriteOffODataEntity> GetDataAsync(long manyQuery, NoneQuery query = null)
        {
            ...
            return _context.FD_BaddebtWriteOffDataSet.FromSqlInterpolated(sql).FirstOrDefaultAsync();
        }

        public override Task<List<FD_BaddebtWriteOffDetailODataEntity>> GetDetaiDatasAsync(long manyQuery)
        {
            var sql = GetDetailSQL();
            sql+=string.Format(@"                                   
                                    where FD.NumericalOrder = {0}", manyQuery);

            return _context.FD_BaddebtWriteOffDetailDataSet.FromSqlRaw(sql).ToListAsync();
        }
        ...
        private string GetDetailSQL()
        {
            return @"SELECT 
                                    ...
                                    IFNULL(FD.WriteOffAmount,0.0)  WriteOffAmount,
                                    CONVERT(FD.ModifiedDate USING utf8mb4) AS ModifiedDate,
                                    dc.DataDictName BusinessTypeName,
                                    IFNULL(bc.CustomerName,hr.Name )   CurrentUnitName
                                    FROM  nxin_qlw_business.FD_BaddebtWriteOffdetail FD 
