...
namespace FinanceManagement.ApiHost.Controllers.FD_BaddebtRecovery
{
    ...
    /// 增加
    /// </summary>
    public class FD_BaddebtRecoveryAddHandler : IRequestHandler<FD_BaddebtRecoveryAddCommand, Result>
    {
        IIdentityService _identityService;
        IFD_BaddebtRecoveryRepository _repository;
        IFD_BaddebtRecoveryDetailRepository _detailRepository;
        NumericalOrderCreator _numericalOrderCreator;
        NumberCreator<Nxin_Qlw_BusinessContext> _numberCreator;
        IBiz_ReviewRepository _biz_ReviewRepository;

        public FD_BaddebtRecoveryAddHandler(IIdentityService identityService, IFD_BaddebtRecoveryRepository repository, IFD_BaddebtRecoveryDetailRepository detailRepository, NumericalOrderCreator numericalOrderCreator, NumberCreator<Nxin_Qlw_BusinessContext> numberCreator, IBiz_ReviewRepository biz_ReviewRepository)
        {
        ...
        public async Task<Result> Handle(FD_BaddebtRecoveryAddCommand request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                long number = _numberCreator.Create<Domain.FD_BaddebtRecovery>(request.DataDate, o => o.Number, o => o.Number.StartsWith(request.DataDate.ToString("yyyyMMdd")), o => o.EnterpriseID == request.EnterpriseID); 
                var numericalOrder = await _numericalOrderCreator.CreateAsync();
                var domain = new Domain.FD_BaddebtRecovery()
                {
                ...
                foreach (var item in request.Lines)
                {
                    ...
                    var detial = new FD_BaddebtRecoveryDetail()
                    {
                        NumericalOrder = numericalOrder,
    ...
    /// 删除
    /// </summary>
    public class FD_BaddebtRecoveryDeleteHandler : IRequestHandler<FD_BaddebtRecoveryDeleteCommand, Result>
    {
        IIdentityService _identityService;
        IFD_BaddebtRecoveryRepository _repository;
        IFD_BaddebtRecoveryDetailRepository _detailRepository;
        IBiz_ReviewRepository _biz_ReviewRepository;
        public FD_BaddebtRecoveryDeleteHandler(IIdentityService identityService, IFD_BaddebtRecoveryRepository repository, IFD_BaddebtRecoveryDetailRepository detailRepository, IBiz_ReviewRepository biz_ReviewRepository)
        {
            _identityService = identityService;
            _repository = repository;
            _detailRepository = detailRepository;
        ...
        public async Task<Result> Handle(FD_BaddebtRecoveryDeleteCommand request, CancellationToken cancellationToken)
        {
            Result result = new Result();
            try
            {
    ...
    /// 删除
    /// </summary>
    public class FD_BaddebtRecoveryDeleteDetailsHandler : IRequestHandler<FD_BaddebtRecoveryDeleteDetailCommand, Result>
    {
        IIdentityService _identityService;
        IFD_BaddebtRecoveryRepository _repository;
        IFD_BaddebtRecoveryDetailRepository _detailRepository;
        IBiz_ReviewRepository _biz_ReviewRepository;
        public FD_BaddebtRecoveryDeleteDetailsHandler(IIdentityService identityService, IFD_BaddebtRecoveryRepository repository, IFD_BaddebtRecoveryDetailRepository detailRepository, IBiz_ReviewRepository biz_ReviewRepository)
        {
            _identityService = identityService;
            _repository = repository;
            _detailRepository = detailRepository;
        ...
        /// <returns></returns>
        public async Task<Result> Handle(FD_BaddebtRecoveryDeleteDetailCommand request, CancellationToken cancellationToken)
        {
            Result result = new Result();
            try
            {
    ...
    public class FD_BaddebtRecoveryModifyHandler : IRequestHandler<FD_BaddebtRecoveryModifyCommand, Result>
    {
        IIdentityService _identityService;
        IFD_BaddebtRecoveryRepository _repository;
        IFD_BaddebtRecoveryDetailRepository _detailRepository;
        NumericalOrderCreator _numericalOrderCreator;
        public FD_BaddebtRecoveryModifyHandler(IIdentityService identityService, IFD_BaddebtRecoveryRepository repository, IFD_BaddebtRecoveryDetailRepository detailRepository,
        ...
        public async Task<Result> Handle(FD_BaddebtRecoveryModifyCommand request, CancellationToken cancellationToken)
        {
            Result result = new Result();
            try
            {
                ...
                foreach (var item in request.Lines)
                {
                    ...
                    if (item.RowStatus == "M" || item.IsUpdate)
                    {
                        ...
                        obj?.Update(item.BusiType,  item.BusinessType, item.CurrentUnit, item.Amount,item.RecoveryAmount);                        
                    }
                }
