...
{
    public class FD_BadDebtRecoverAddHandler : IRequestHandler<FD_BadDebtRecoverAddCommand, Result>
    {
        IIdentityService _identityService;
        IFD_BadDebtRecoverRepository _repository;
        IFD_BadDebtRecoverDetailRepository _detailRepository;
        NumericalOrderCreator _numericalOrderCreator;
        NumberCreator<Nxin_Qlw_BusinessContext> _numberCreator;
        IBiz_ReviewRepository _biz_ReviewRepository;

        public FD_BadDebtRecoverAddHandler(IIdentityService identityService, IFD_BadDebtRecoverRepository repository, IFD_BadDebtRecoverDetailRepository detailRepository, NumericalOrderCreator numericalOrderCreator, NumberCreator<Nxin_Qlw_BusinessContext> numberCreator, IBiz_ReviewRepository biz_ReviewRepository)
        {
        ...
        public async Task<Result> Handle(FD_BadDebtRecoverAddCommand request, CancellationToken cancellationToken)
        {

            Result result = new Result();
            try
            {
                long number = _numberCreator.Create<Domain.FD_BadDebtRecover>(o => o.DataDate, o => o.Number, Convert.ToDateTime(request.DataDate), o => o.EnterpriseID == _identityService.EnterpriseId);
                var numericalOrder = await _numericalOrderCreator.CreateAsync();
                var domain = new Domain.FD_BadDebtRecover()
                {
                ...
                request.Lines1?.ForEach(o =>
                    {
                        domain.Lines.Add(new FD_BadDebtRecoverDetail()
                        {
                            NumericalOrder = numericalOrder,
                            ...
                            PersonID = o.PersonID,
                            Amount = o.Amount,
                            CurrentRecoverAmount = o.CurrentRecoverAmount,
                        });
                    });
    ...
    public class FD_BadDebtRecoverDeleteHandler : IRequestHandler<FD_BadDebtRecoverDeleteCommand, Result>
    {
        IIdentityService _identityService;
        IFD_BadDebtRecoverRepository _repository;
        IFD_BadDebtRecoverDetailRepository _detailRepository;

        public FD_BadDebtRecoverDeleteHandler(IIdentityService identityService, IFD_BadDebtRecoverRepository repository, IFD_BadDebtRecoverDetailRepository detailRepository)
        {
            _identityService = identityService;
            _repository = repository;
            _detailRepository = detailRepository;
        ...
        public async Task<Result> Handle(FD_BadDebtRecoverDeleteCommand request, CancellationToken cancellationToken)
        {
            Result result = new Result();
            try
            {
    ...
    public class FD_BadDebtRecoverModifyHandler : IRequestHandler<FD_BadDebtRecoverModifyCommand, Result>
    {
        IIdentityService _identityService;
        IFD_BadDebtRecoverRepository _repository;
        IFD_BadDebtRecoverDetailRepository _detailRepository;
        public FD_BadDebtRecoverModifyHandler(IIdentityService identityService, IFD_BadDebtRecoverRepository repository, IFD_BadDebtRecoverDetailRepository detailRepository)
        {
            _identityService = identityService;
            _repository = repository;
            _detailRepository = detailRepository;
        ...
        public async Task<Result> Handle(FD_BadDebtRecoverModifyCommand request, CancellationToken cancellationToken)
        {
            Result result = new Result();
            try
            {
                ...
                domain.Lines.ForEach(o =>
                {
                    ...
                    if (item != null)
                    {
                        ...
                        o.CurrentRecoverAmount = item.CurrentRecoverAmount;
