...
{
    public class FA_AssetsContractODataProvider : OneWithManyQueryProvider<FA_AssetsContractODataEntity, FA_AssetsContractDetailODataEntity>
    {
        ...
        public FA_AssetsContractODataProvider(IIdentityService identityservice, QlwCrossDbContext context, BIZ_DataDictODataProvider biz_DataDictODataProvider, IBiz_Related biz_RelatedRepository)
        {
        ...
        public IEnumerable<FA_AssetsContractListODataEntity> GetList(ODataQueryOptions<FA_AssetsContractListODataEntity> odataqueryoptions, Uri uri)
        {
            var datas = GetData(odataqueryoptions, uri).AsEnumerable();
            return datas;
        }
        public IQueryable<FA_AssetsContractListODataEntity> GetData(ODataQueryOptions<FA_AssetsContractListODataEntity> odataqueryoptions, Uri uri)
        {
            return GetMainList();
        }
        /// <summary>
        ...
        /// <returns></returns>
        public IQueryable<FA_AssetsContractListODataEntity> GetMainList(NoneQuery query = null)
        {
            FormattableString sql = $@" SELECT CONVERT(t1.NumericalOrder USING utf8mb4) NumericalOrder,	
                                        ...
                                        t1.ContractName,
                                        CONVERT(t1.ContractNumber USING utf8mb4) ContractNumber,	
                                        ...
                                        CONVERT(t1.ContractTemplate USING utf8mb4) ContractTemplate,
                                        dict.DataDictName AS ContractTemplateName,
                                        CONVERT(t1.OwnerID USING utf8mb4) OwnerID,
                                        HP.Name AS OwnerName,
                                        t1.Remarks,	
                                        SUM(ifnull(t2.Quantity,0)) AS Quantity,
                                        ...
                                        FROM `nxin_qlw_business`.FA_AssetsContract t1
                                        LEFT JOIN `nxin_qlw_business`.FA_AssetsContractDetail t2 ON t1.`NumericalOrder`=t2.`NumericalOrder`
                                         LEFT JOIN (select * from `nxin_qlw_business`.`pm_supplier` where EnterpriseID = {_identityservice.EnterpriseId} group by SupplierID)  sup ON t1.SupplierID=sup.SupplierID
                                        ...
                                        LEFT JOIN nxin_qlw_business.biz_datadict dict ON t1.ContractTemplate=dict.DataDictID
                                        ...
                                        inner join `nxin_qlw_business`.FA_AssetsContract temp on temp.NumericalOrder = faac.NumericalOrder
                                        ...
                                        GROUP BY t2.NumericalOrder
                                        ORDER BY t1.`DataDate` DESC";
            return _context.FA_AssetsContractListDataSet.FromSqlInterpolated(sql);
        }
        public override IQueryable<FA_AssetsContractODataEntity> GetDatas(NoneQuery query = null)
        {
            throw new NotImplementedException();
        }
        public override Task<FA_AssetsContractODataEntity> GetDataAsync(long manyQuery, NoneQuery query = null)
        {
            FormattableString sql = $@"SELECT CONVERT(t1.NumericalOrder USING utf8mb4) NumericalOrder,	
                                        CONVERT(DATE_FORMAT( t1.DataDate,'%Y-%m-%d') USING utf8mb4) DataDate,
                                        t1.ContractName,
                                        ...
                                        CONVERT(t1.ContractClause USING utf8mb4) ContractClause,
            ...
            var main = _context.FA_AssetsContractDataSet.FromSqlInterpolated(sql).FirstOrDefault();
            var detail = this.GetDetaiDatasAsync(manyQuery).Result;
            if (detail.Count > 0)
            {
                string imputStr = string.Empty;
                detail.ForEach(s =>
                {
                    if (!string.IsNullOrEmpty(s.NumericalOrderInput)) imputStr += (s.NumericalOrderInput+",");
                });
                if (!string.IsNullOrEmpty(imputStr))
                {
                    imputStr = imputStr.TrimEnd(',');
                    //List<FA_AssetsApplyDetailODataEntity> applys = GetApplyDetails(imputStr);
                    main.ApplyForms = imputStr; //string.Join(',', applys.Select(s => s.NumericalOrder));
                }
                main.Lines = detail;
            }
            return Task.FromResult(main);
        }
        public override Task<List<FA_AssetsContractDetailODataEntity>> GetDetaiDatasAsync(long manyQuery)
        {
            FormattableString sql = $@"SELECT CONVERT(t1.NumericalOrder USING utf8mb4) NumericalOrder,	
                                        ...
                                        t1.Remarks
                                        FROM `nxin_qlw_business`.FA_AssetsContractDetail t1
                                        LEFT JOIN nxin_qlw_business.fa_assetsclassification  cl ON t1.AssetsTypeID=cl.ClassificationID
                                        LEFT JOIN `qlw_nxin_com`.`unitmeasurement` un ON t1.MeasureUnit=un.UnitID
                                        WHERE t1.NumericalOrder={manyQuery}";
            var result = _context.FA_AssetsContractDetailDataSet.FromSqlInterpolated(sql).ToListAsync();
            return result;
        }
        ...
        /// 资产验收单引用的资产购置合同
        /// </summary>
        public IQueryable<FA_AssetsContractMobileListODataEntity> GetAssetsContractMobileListByInspect(ODataQueryOptions<FA_AssetsContractMobileListODataEntity> odataqueryoptions, Uri uri)
        {
            FormattableString sql = $@" SELECT CONVERT(t1.NumericalOrder USING utf8mb4) NumericalOrder,	
						...
						GROUP BY NumericalOrderDetail
                                        ...
                                        ORDER BY t1.`DataDate` DESC";
            return _context.FA_AssetsContractMobileListDataSet.FromSqlInterpolated(sql);
        }
      
        public  List<FA_AssetsApplyDetailODataEntity> GetApplyDetails(string manyQuery)
        {
            string sql = $@"SELECT CONVERT(t1.NumericalOrder USING utf8mb4) NumericalOrder,	
                                        ...
                                        WHERE t1.NumericalOrderDetail in ({manyQuery})
                                        GROUP BY t1.NumericalOrder";
            var result = _context.FA_AssetsApplyDetailDataSet.FromSqlRaw(sql).ToList();
            return result;
        }
    }
}
